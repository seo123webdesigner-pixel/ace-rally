...
| Thing | Choice |
|---|---|
| Engine | Godot 4.7.2 stable, pinned. Do not upgrade without a dedicated task |
...
You have terminal access on the human's machine. Godot is installed and on PATH. You can run the parse
check, the full GUT suite, and any headless harness you write. You can read log output.

You **cannot see the game window.** Godot renders to a native window that you have no view into. This is
the single most important constraint on how you report your work.

Rules that follow:

- **Never claim a thing "looks right", "feels good" or "works well".** You did not see it. State what the
  tests prove and stop there.
- **Anything requiring visual or tactile judgement goes into `NEEDS_HUMAN_CHECK.md`** as a numbered list:
  what to look at, and what correct would look like. The human clears that list and reports back.
- **Every gameplay system needs a headless test.** If you cannot test it headlessly, push the logic into
  a pure class in `scripts/sim/` until you can. That constraint is deliberate.
- **Prefer logged numbers over descriptions.** When something needs diagnosis, write a harness that dumps
  a CSV the human can read, rather than asking them to describe what they saw.
- **Scene files:** you may edit .tscn as text. Always run the parse check afterwards, and prefer a test
  that instantiates the scene over assuming it is valid.
- **Android export is a Sprint 12 task only.** Do not attempt it early.
...
# Android release build (Sprint 12 only)
godot --headless --path . --export-release "Android" build/acerally.apk
```

On Windows, run these through Git Bash or use `scripts/verify.ps1` if you generate a PowerShell variant.

---

## 11. Definition of done for any sprint

1. `bash scripts/verify.sh` exits 0. Paste the real output, never a summary of it.
2. All GUT tests pass.
3. Every acceptance criterion is either proven by a test, or listed in `NEEDS_HUMAN_CHECK.md` for the
   human to verify on their machine. Nothing is silently assumed.
4. `NOTES.md` updated with what changed, what was deferred and any tuning values that need a human eye.
5. Committed with a clean message and pushed.
